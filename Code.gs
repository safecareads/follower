...
function doGet(e) {
  ...
  if (e && e.parameter && e.parameter.action === 'checkTransactionUsed') {
    return handleCheckTransactionUsedRequest_(e);
  }
  if (e && e.parameter && e.parameter.action === 'orderStatus') {
    return handleOrderStatusRequest_(e);
  }
...
function doPost(e) {
  try {
    ...
    const generatedOrderId = 'SC-' + Utilities.formatDate(
      timestamp,
      Session.getScriptTimeZone() || 'Asia/Dhaka',
      'yyyyMMdd-HHmmss'
    ) + '-' + Math.floor(Math.random()*900+100);
    const orderId = /^SC-[A-Za-z0-9_-]{6,80}$/.test(clientOrderId)
      ? clientOrderId
      : generatedOrderId;
    ...
    // Block reuse of a Transaction ID that already belongs to an order.
    // This check is intentionally separate from payment verification:
    // verification only asks whether the Transaction ID exists in the SMS log,
    // while order submission must ensure the same Transaction ID is not used twice.
    const lock = LockService.getScriptLock();
    lock.waitLock(10000);
    try {
      const used = findUsedTransactionId_(sheet, transactionId);
      if (used.found) {
        return htmlResponse_(
          '<script>window.parent.postMessage({type:"safe-care-order",status:"duplicate",orderId:' +
          JSON.stringify(used.orderId || '') + ',transactionId:' + JSON.stringify(transactionId) + '}, "*");</script>' +
          '<div style="font-family:Arial,sans-serif;padding:30px;color:#8a4b08">' +
          '<h2>Transaction ID already used</h2><p>এই Transaction ID দিয়ে আগেই একটি Order নেওয়া হয়েছে।</p>' +
          '<p>Order ID: <b>' + escapeHtml_(used.orderId || '') + '</b></p></div>'
        );
      }
...
function handleVerifyPaymentRequest_(e) {
  ...
  ensureHeaders_(orderSheet);
  const used = findUsedTransactionId_(orderSheet, transactionId);
  result.used = !!used.found;
  result.usedOrderId = used.orderId || '';
  if (used.found) {
    result.reason = 'Payment Confirmed, কিন্তু এই Transaction ID দিয়ে আগেই একটি Order নেওয়া হয়েছে।';
  }
  return verifyResponse_(result, e && e.parameter ? e.parameter.callback : '');
}

function handleCheckTransactionUsedRequest_(e) {
  let transactionId = '';
  try {
    const p = (e && e.parameter) || {};
    transactionId = normalizeTxn_(p.transactionId || '');
  ...
  if (!transactionId || transactionId.length < 4) {
    return verifyResponse_({used:false, orderId:'', reason:'Transaction ID দিন।'}, e && e.parameter ? e.parameter.callback : '');
  }

  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  const sheet = getOrCreateSheet_(ss, SHEET_NAME);
  ensureHeaders_(sheet);
  const result = findUsedTransactionId_(sheet, transactionId);
  return verifyResponse_({
    verified: !result.found,
    used: !!result.found,
    orderId: result.orderId || '',
    reason: result.found
      ? 'এই Transaction ID দিয়ে আগেই একটি Order নেওয়া হয়েছে।'
      : 'এই Transaction ID এখনো কোনো Order-এ ব্যবহার হয়নি।'
  }, e && e.parameter ? e.parameter.callback : '');
}

function findUsedTransactionId_(sheet, transactionId) {
  const wanted = normalizeTxn_(transactionId);
  if (!wanted || sheet.getLastRow() < 2) return {found:false, orderId:''};
  const idx = headerIndexes_(sheet);
  const values = sheet.getRange(2, 1, sheet.getLastRow()-1, sheet.getLastColumn()).getValues();
  for (let i = values.length - 1; i >= 0; i--) {
    const rowTxn = normalizeTxn_(values[i][idx.transactionId] || '');
    if (rowTxn && rowTxn === wanted) {
      return {found:true, orderId:String(values[i][idx.orderId] || '')};
    }
  }
  return {found:false, orderId:''};
}

function handleOrderStatusRequest_(e) {
  const p = (e && e.parameter) || {};
  const orderId = String(p.orderId || '').trim();
  if (!orderId) {
    return verifyResponse_({found:false, orderId:'', status:'', reason:'Order ID missing'}, p.callback || '');
  }
  ...
  if (sheet.getLastRow() < 2) {
    return verifyResponse_({found:false, orderId:orderId, status:'', reason:'Order এখনো পাওয়া যায়নি।'}, p.callback || '');
  }

  const idx = headerIndexes_(sheet);
  const values = sheet.getRange(2, 1, sheet.getLastRow()-1, sheet.getLastColumn()).getValues();
  for (let i = values.length - 1; i >= 0; i--) {
    if (String(values[i][idx.orderId] || '').trim() === orderId) {
      return verifyResponse_({
        found:true,
        orderId:orderId,
        status:String(values[i][idx.status] || ''),
        reason:'Order status পাওয়া গেছে।'
      }, p.callback || '');
    }
  }
...
function verifyResponse_(result, callback) {
  const data = {
    ...
    orderId: result.orderId || result.usedOrderId || '',
    reason: result.reason || ''
  };
  if (Object.prototype.hasOwnProperty.call(result, 'used')) data.used = !!result.used;
  if (Object.prototype.hasOwnProperty.call(result, 'found')) data.found = !!result.found;
  if (Object.prototype.hasOwnProperty.call(result, 'status')) data.status = result.status || '';
